...
const VendorManagementPage = () => {
   const [searchTerm, setSearchTerm] = useState('');
    const [syncedVendors, setSyncedVendors] = useState(['Amazon', 'Nike', 'Shopify', 'Walmart']); 
    const [selectedVendor, setSelectedVendor] = useState('Amazon'); 
    const [copiedCode, setCopiedCode] = useState<string | null>(null);
    
    const currentRewards = VENDOR_REWARDS[selectedVendor] || [];

    const filteredVendors = VENDORS.filter(vendor =>
        vendor.toLowerCase().includes(searchTerm.toLowerCase()) && 
        !syncedVendors.includes(vendor)
    );
    ...
    const handleVendorSync = (vendorName: string) => {
                        ...
                        <div className="absolute z-10 w-full mt-1 bg-gray-800 rounded-lg shadow-xl border border-gray-700 max-h-60 overflow-y-auto">
                            {filteredVendors.length > 0 ? (
                                ...
                                <div className="p-3 text-gray-400">
                                    {syncedVendors.includes(searchTerm) ? 
            ...
            <main className="flex max-w-5xl mx-auto border-t border-gray-800 pt-6">
                ...
                <section className="w-1/3 pr-6">
                    ...
                    <div className="space-y-3">
                        {syncedVendors.map((vendor, index) => (
                            <div
                                key={index}
                            ...
                            >
                                <span onClick={() => handleVendorClick(vendor)} className="flex-grow">
                                    {vendor}
                                </span>
                ...
                <section className="w-2/3 pl-6">
                    <h2 className="text-xl font-bold mb-4">Rewards/Coupons for {selectedVendor || 'No Vendor Selected'}</h2>
                    <div className="space-y-4">
                        {currentRewards.length > 0 ? (
                            currentRewards.map(reward => (
                                <div key={reward.code} className="p-4 bg-gray-800 rounded-lg shadow-lg flex justify-between items-center">
                                    <div className="flex flex-col">
                                        <div className="text-2xl font-extrabold text-white tracking-wider mb-1">{reward.code}</div>
                                        ...
                                        {copiedCode === reward.code ? (
                                            ...
                                            >
                                                <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 5H6a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2v-1M8 5a2 2 0 002 2h2a2 2 0 002-2M8 5a2 2 0 012-2h2a2 2 0 012 2m0 0h2a2 2 0 012 2v3m-3 3h4a2 2 0 002-2v-4a2 2 0 00-2-2h-8a2 2 0 00-2 2v4a2 2 0 002 2z" />
                                                </svg>
                                            </button>
                                        )}
                                    </div>
                            ...
                            <p className="text-gray-400 p-4 bg-gray-800 rounded-lg">No active rewards found for {selectedVendor}.</p>
                        )}
                    </div>
                </section>
            </main>
